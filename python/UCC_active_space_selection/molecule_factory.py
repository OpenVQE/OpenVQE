...
class MoleculeFactory:
    ...
    def get_reference_ket(self, hf_init, nbqbits, transform):
        ...
        referenceState = self.from_ket_to_vector(list_ket_hf)
        sparseReferenceState = scipy.sparse.csr_matrix(
            referenceState, dtype=complex
        ).transpose()
        return sparseReferenceState, hf_init_sp
    ...
    def generate_cluster_ops(
        ...
        if active == True:
            ...
            n_orb = int(len(active_orb_energies) / 2)
        ...
        if type_of_generator == "singlet_sd":
            ...
            pool_size, cluster_ops, cluster_ops_sp = singlet_upccgsd(n_orb, transform)
            ...
            (
                ...
                theta_MP2,
                hf_init,
            ) = self.calculate_uccsd(molecule_symbol, transform, active=True)
            return pool_size, cluster_ops, cluster_ops_sp, theta_MP2, hf_init
        else:
            return
