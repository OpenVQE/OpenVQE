...
class FSUCCOperator (object):
    ...
    def __init__(self, norb, a_idxs, i_idxs):
        ...
        # Filtering duplicates from a_idxs and i_idxs
        valid_a_idxs = []
        valid_i_idxs = []
        #print("a_idxs",a_idxs)
        for a, i in zip(a_idxs, i_idxs):
            a_array, i_array = np.asarray(a, dtype=np.uint8), np.asarray(i, dtype=np.uint8)
            if a_array.size and i_array.size:  # Ensure they are valid arrays
                if len(np.unique(a_array.flatten())) == len(a_array.flatten()) and len(np.unique(i_array.flatten())) == len(i_array.flatten()):
                    valid_a_idxs.append(a_array)
                    valid_i_idxs.append(i_array)

        self.a_idxs = [np.ascontiguousarray(a, dtype=np.uint8) for a in valid_a_idxs]
        self.i_idxs = [np.ascontiguousarray(i, dtype=np.uint8) for i in valid_i_idxs]
        #print("valid_a_idxs",valid_a_idxs)
