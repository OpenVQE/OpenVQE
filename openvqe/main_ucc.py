...
hamiltonian, hamiltonian_sp, \
    n_elec, noons_full, orb_energies_full, info = molecule_factory.generate_hamiltonian(
        molecule_symbol, active=active, transform=transform)
...
# for uccsd_case
# pool_size,cluster_ops,cluster_ops_sp, theta_MP2, hf_init = \
#    molecule_factory.generate_cluster_ops(molecule_symbol,
...
# suppose user wants to generate cluster operators
#   using the Qubit Pool generators: then we can have two functions,
# 1. generate_pool_without_cluster
...
len_returned_pool, returned_pool = \
    qubit_pool.generate_pool_from_cluster(pool_condition, cluster_ops, nbqbits)


# HERE USER can apply UCC-family VQE using MP2 guess defined by theta_current1;
#   and using either a constant value starting guess
...
# define two lists:
# 1. ansatz_ops to append operators direct coming from
#   fermionic evolutions (after spin-transformation)
# 2. ansatz_q_ops to append operators coming from "Qubit Pool generators"
ansatz_ops = []
ansatz_q_ops = []

# important note: when UCCSD  is called at the top,
#   then we must skip 1j in ansatz_ops
#   (why? due to the internal functions defined by "mqlm-fermion" package)
...
# 2. from  UCC-family but with qubit evolutions
#   i.e from "Qubit Pool" (with ansatz_q_ops and theta-current2)

iterations, result = energy_ucc.get_energies(hamiltonian_sp,ansatz_ops,ansatz_q_ops,
                                             hf_init_sp,theta_current1,theta_current2,fci)
