...
class UCC(Algorithm):
    ...
    def generate_pool_from_cluster(self, cluster_ops, nbqbits):
        print(" --------------------------------------------------------------------------")
        print("                                                          ")
    ...
    def get_ansatz(self, cluster_ops_sp, returned_pool):
    ...
    def execute(self):
        ...
        hamiltonian, hamiltonian_sp, n_elec, noons_full, orb_energies_full, info = self.generate_hamiltonian(molecule_factory)
        _, cluster_ops, cluster_ops_sp = self.generate_cluster_ops(molecule_factory)
        _, returned_pool = self.generate_pool_from_cluster(cluster_ops, hamiltonian_sp.nbqbits)
        hf_init = molecule_factory.find_hf_init(hamiltonian, n_elec, noons_full, orb_energies_full)
        _, hf_init_sp = molecule_factory.get_reference_ket(hf_init, hamiltonian_sp.nbqbits, self.transform)
        ansatz_ops, ansatz_q_ops = self.get_ansatz(cluster_ops_sp, returned_pool)
        # when UCCSD is not used then one can make theta_current1 same as theta_current2     
        theta_current = []
        for i in range(len(returned_pool)):
            theta_current.append(self.opts['step'])
