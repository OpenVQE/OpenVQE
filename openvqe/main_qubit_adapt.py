...
print(" --------------------------------------------------------------------------")
print("                                                          ")
hamiltonian, hamiltonian_sparse, hamiltonian_sp, hamiltonian_sp_sparse,\
...
pool_size, cluster_ops, cluster_ops_sp, cluster_ops_sparse = \
...
# user can just type the name of pool wanted:
#       full, full_without_Z, reduced_without_Z, YXXX, XYXX,XXYX,XXXY,random, two, four, eight
# pure_with_symmetry

# for example here user can put radom (YXXX, XYXX, XXYX, XXXY):
pool_type = 'random'
qubit_pool =qubitpool.generate_pool(cluster_ops)
len_returned_pool, returned_pool = qubitpool.generate_pool_without_cluster(
    pool_type=pool_type, nbqbits=nbqbits, qubit_pool=qubit_pool, molecule_symbol=molecule_symbol)
# or user can type:
# pool_condition='full_without_Z'
# len_returned_pool, returned_pool = 
#       qubitpool.generate_hamiltonian_from_cluster(pool_condition, cluster_ops, nbqbits)
...
iterations_sim, iterations_ana, result_sim, result_ana = qubit_adapt_vqe(
    hamiltonian_sp, hamiltonian_sp_sparse,
    reference_ket, nbqbits, pool_mix, hf_init_sp, info['FCI'],
    chosen_grad = 1,
    adapt_conver    = 'norm', adapt_thresh    = 1e-07, adapt_maxiter   = 29,
    tolerance_sim = 1e-09,
    method_sim = 'BFGS')
